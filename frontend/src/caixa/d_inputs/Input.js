...
export function InputText({ width, flex, title, name, placeholder, handleOnChange, value }) {
    return (
        <div className='inputContainer1' style={flex ? { flexDirection: flex } : { alignItems: 'center' }}>
...
export function Select({ padrao, width, flex, text, name, options, handleOnChange, value }) {
    ...
    return (
        <div id='inputContainer1' style={flex ? { flexDirection: flex } : { alignItems: 'center' }}>
            ...
            >
                ...
                {options.map((option) => (
               option.id && <option value={option.id} name={option.nome} key={option.id}>{option.nome}</option>
                ))}
            </select>
        </div>
    )
