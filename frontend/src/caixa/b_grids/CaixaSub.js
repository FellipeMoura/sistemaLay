...
import { InputCash, Button, Select } from "../d_inputs/Input";
import {VscSave} from 'react-icons/vsc';
function CaixaSub({ admin, setIsSub, dataCards, setRefresh, refresh }) {
    ...
    function RowSub({ dataCard }) {
        ...
        return (
            state ? (
                <tr className='' key={dataCard.venda_sub} onClick={()=> admin ? setState(false): console.log('nops')}>
        ...
        return (
            card.valor ?
                state ? (
                    ...
                    <tr className='hover' key={card.id} onClick={() => setToEdit() }>
                        

                        <td>{card.nome_forma}</td>
                        <td>{parseFloat(card.valor).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td>
                        <td>{card.nome_itens}</td>
                    </tr>
                ) : (
